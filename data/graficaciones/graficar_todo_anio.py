...
def graficaranio(directory, airport, an, delay_filter, data):
...
#       file_out_data.write(str(val) + '\n')
  ...
  for h in range(0, ultimo_anio - primer_anio + 1):
    ...
    for k in range(0, 12*(ultimo_anio - primer_anio + 1)):
      ...
      plt.axvline(x=15*k*7, linewidth=1, color='k')
...
#     print eje_x['x_lunes_' + str(h)]
    ...
    for z in range(0, 12):
      ...
      for d in dias:      
        eje_x['x_' + d + '_' + str(h)] = [i for i in range(aux, aux+15)]
        aux += 15
...
#       print eje_x['x_domingo_' + str(h)]
    ...
    plt.xticks([7.5+7*z for z in range(0, 12*(ultimo_anio - primer_anio + 1)*7)],labels,fontsize=10)
    ...
    directory = directory + '/graficos_año'

    if not os.path.exists(directory):
      os.mkdir(directory)

    plt.savefig(directory + '/año' + "2006" )
