...
def graficaranio2(directory, airport, an, delay_filter, data):
...
# Instancio los arreglos para los datos, del 2005 al 2008
  ...
  for l in range(primer_anio, ultimo_anio + 1):
    ...
    intervalo_lunes     = [ [0] for j in range(0, 12)]
    intervalo_martes    = [ [0] for j in range(0, 12)]
    intervalo_miercoles = [ [0] for j in range(0, 12)]
    intervalo_jueves    = [ [0] for j in range(0, 12)]
    intervalo_viernes   = [ [0] for j in range(0, 12)]
    intervalo_sabado    = [ [0] for j in range(0, 12)]
    intervalo_domingo   = [ [0] for j in range(0, 12)]
...
#       file_out_data.write(str(val) + '\n')
  ...
  for h in range(0, ultimo_anio - primer_anio + 1):
    ...
    for k in range(0, 12*(ultimo_anio - primer_anio + 1)):
      #plt.axvline(x=18*k, linewidth=1, color='k')
      plt.axvline(x=k*7, linewidth=1, color='k')
...
#     print eje_x['x_lunes_' + str(h)]
    ...
    for z in range(0, 12):
      ...
      for d in dias:      
        eje_x['x_' + d + '_' + str(h)] = [i for i in range(aux, aux+1)]
        aux += 1

      if z == 0:
        print len(anio_lunes[h][z])
        print len(eje_x['x_lunes_' + str(h)])
        plt.plot(eje_x['x_lunes_' + str(h)], anio_lunes[h][z] , 'ro', 
        alpha=opacity,
        color='b',
        linestyle='-',
        label=u"Lunes")
        valsX += eje_x['x_lunes_' + str(h)]
        valsY += anio_lunes[h][z] 

#       print eje_x['x_martes_' + str(h)]
        plt.plot(eje_x['x_martes_' + str(h)], anio_martes[h][z] , 'ro', 
        alpha=opacity,
        color='g',
        linestyle='-',
        label=u"Martes")
        valsX += eje_x['x_martes_' + str(h)]
        valsY += anio_martes[h][z] 

#       print eje_x['x_miercoles_' + str(h)]
        plt.plot(eje_x['x_miercoles_' + str(h)], anio_miercoles[h][z] , 'ro', 
        alpha=opacity,
        color='r',
        linestyle='-',
        label=u"Miercoles")
        valsX += eje_x['x_miercoles_' + str(h)]
        valsY += anio_miercoles[h][z] 

#       print eje_x['x_jueves_' + str(h)]
        plt.plot(eje_x['x_jueves_' + str(h)], anio_jueves[h][z] , 'ro', 
        alpha=opacity,
        color='c',
        linestyle='-',
        label=u"Jueves")
        valsX += eje_x['x_jueves_' + str(h)]
        valsY += anio_jueves[h][z] 

#       print eje_x['x_martes_' + str(h)]
        plt.plot(eje_x['x_viernes_' + str(h)], anio_viernes[h][z] , 'ro', 
        alpha=opacity,
        color='m',
        linestyle='-',
        label=u"Viernes")
        valsX += eje_x['x_viernes_' + str(h)]
        valsY += anio_viernes[h][z] 

#       print eje_x['x_sabado_' + str(h)]
        plt.plot(eje_x['x_sabado_' + str(h)], anio_sabado[h][z] , 'ro', 
        alpha=opacity,
        color='y',
        linestyle='-',
        label=u"Sabado")
        valsX += eje_x['x_sabado_' + str(h)]
        valsY += anio_sabado[h][z] 

#       print eje_x['x_domingo_' + str(h)]
        plt.plot(eje_x['x_domingo_' + str(h)], anio_domingo[h][z] , 'ro', 
        alpha=opacity,
        color='k',                                                                                                  linestyle='-',
        label=u"Domingo")
        valsX += eje_x['x_domingo_' + str(h)]
        valsY += anio_domingo[h][z] 
...
#       print eje_x['x_domingo_' + str(h)]
    ...
    plt.xticks([0.5+7*z for z in range(0, 12*(ultimo_anio - primer_anio + 1))],labels,fontsize=10)
    ...
    directory = directory + '/graficos_año_mes'

    if not os.path.exists(directory):
      os.mkdir(directory)

    plt.savefig(directory + '/año' + "2004" )
